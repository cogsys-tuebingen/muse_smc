...
public:
    ...
    void addPrediction(const ros::Time &time,
...
private:
    ...
    inline std::string toString(const ros::Time &time)
    {
        long milli = std::floor(time.toNSec() / 1e6);
        long s  = milli / 1000;
        long ms = milli % 1000;
        const std::string ms_off = ms >= 100 ? "" : (ms >= 10 ? "0" : "00");
