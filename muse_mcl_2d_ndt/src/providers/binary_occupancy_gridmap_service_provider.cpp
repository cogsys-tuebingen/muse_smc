#include <muse_mcl_2d_ndt/providers/binary_occupancy_gridmap_service_provider.h>
...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_ndt::BinaryOccupancyGridmapServiceProvider, muse_mcl_2d::MapProvider2D)

namespace muse_mcl_2d_ndt {
BinaryOccupancyGridmapServiceProvider::BinaryOccupancyGridmapServiceProvider() :
    loading_(false)
{
}

BinaryOccupancyGridmapServiceProvider::state_space_t::ConstPtr BinaryOccupancyGridmapServiceProvider::getStateSpace() const
{
    nav_msgs::GetMap req;
    if (source_.call(req))
        loadMap();
...
void BinaryOccupancyGridmapServiceProvider::setup(ros::NodeHandle &nh)
{
    ...
    const std::string topic = nh.param<std::string>(param_name("topic"), "/muse_mcl_2d_ndt/binary_occ_ndt_map");
    pub_ = nh.advertise<nav_msgs::OccupancyGrid>(topic, 1);

    source_ = nh.serviceClient<nav_msgs::GetMap>(service_name_);
}

void BinaryOccupancyGridmapServiceProvider::loadMap() const
{
    if (!loading_ && !map_) {
        loading_ = true;
        ...
        auto load = [this]() {
            ...
            if (cslibs_ndt_2d::dynamic_maps::load(map, path_)) {
                ...
                cslibs_ndt_2d::conversion::from(map, lf_map, sampling_resolution_, inverse_model_, threshold_);
                if (lf_map) {
...
void BinaryOccupancyGridmapServiceProvider::publishMap() const
{
    if (!map_)
        return;
