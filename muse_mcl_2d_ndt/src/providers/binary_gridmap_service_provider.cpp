#include <muse_mcl_2d_ndt/providers/binary_gridmap_service_provider.h>
...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_ndt::BinaryGridmapServiceProvider, muse_mcl_2d::MapProvider2D)
...
void BinaryGridmapServiceProvider::loadMap() const
{
    if (!loading_ && !map_) {
        loading_ = true;
...
void BinaryGridmapServiceProvider::publishMap() const
{
    if (!map_)
        return;
