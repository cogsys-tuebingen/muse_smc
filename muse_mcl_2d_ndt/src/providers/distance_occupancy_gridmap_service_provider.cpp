#include <muse_mcl_2d_ndt/providers/distance_occupancy_gridmap_service_provider.h>
...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_ndt::DistanceOccupancyGridmapServiceProvider, muse_mcl_2d::MapProvider2D)

namespace muse_mcl_2d_ndt {
DistanceOccupancyGridmapServiceProvider::DistanceOccupancyGridmapServiceProvider() :
    loading_(false)
{
}

DistanceOccupancyGridmapServiceProvider::state_space_t::ConstPtr DistanceOccupancyGridmapServiceProvider::getStateSpace() const
{
    nav_msgs::GetMap req;
    if (source_.call(req))
        loadMap();
...
void DistanceOccupancyGridmapServiceProvider::setup(ros::NodeHandle &nh)
{
    ...
    const std::string topic = nh.param<std::string>(param_name("topic"), "/muse_mcl_2d_ndt/distance_occ_ndt_map");
    pub_ = nh.advertise<nav_msgs::OccupancyGrid>(topic, 1);

    source_ = nh.serviceClient<nav_msgs::GetMap>(service_name_);
}

void DistanceOccupancyGridmapServiceProvider::loadMap() const
{
    if (!loading_ && !map_) {
        loading_ = true;
...
void DistanceOccupancyGridmapServiceProvider::publishMap() const
{
    if (!map_)
        return;
