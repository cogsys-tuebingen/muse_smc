...
void SampleSetPublisher2D::setup(ros::NodeHandle &nh)
{
    ...
    const std::string topic_poses = nh.param<std::string>("topic_poses", "muse_mcl_2d/poses");
    const std::string topic_mean = nh.param<std::string>("topic_mean", "muse_mcl_2d/mean");
    const std::string topic_markers = nh.param<std::string>("topic_markers", "muse_mcl_2d/markers");
    const std::string topic_samples = nh.param<std::string>("topic_samples", "muse_mcl_2d/samples");
    ...
    if(publish_mean_) {
        pub_mean_      = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>(topic_mean, 1);
    }
    if(publish_markers_) {
        pub_markers_   = nh.advertise<visualization_msgs::MarkerArray>(topic_markers, 1);
    }
    if(publish_samples_) {
        publish_samples_ = nh.advertise<muse_mcl_2d::SampleSetMsg>(topic_samples, 1);
    }
}
...
void SampleSetPublisher2D::loop()
{
    ...
    auto create_pose = [](const math::Pose2D &pose)
    {
            ...
            const double  maximum_weight)
    {
        ...
        if(maximum_weight > 0.0) {
            __HSV2RGB__(120.0 * sample.weight / maximum_weight, 1.0, 1.0, m.color.r, m.color.g, m.color.b);
        } else {
    ...
    auto create_sample = [&create_pose]
            (const Sample2D &sample)
    {
        SampleMsg msg;
