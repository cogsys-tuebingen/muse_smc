...
void withParticles()
{
    ...
    muse_mcl_2d::Transform2D m_T_l;
    muse_mcl_2d::Point2D p;
    const double radius = 10.0;
    double angle = 0.0;
    double range = 0.0;
    muse_mcl_2d::Point2D ray_end = start + muse_mcl_2d::Vector2D(std::cos(angle) * radius,
                                                             std::sin(angle) * radius);
    start_time = muse_smc::Time::now();
    for(auto it = iteration.begin() ; it != end ; ++it) {
        muse_mcl_2d::Point2D   ray_end_point = m_T_l * ray_end;
