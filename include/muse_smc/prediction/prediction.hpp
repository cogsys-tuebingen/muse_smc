...
class Prediction {
 public:
  ...
  inline typename prediction_model_t::Result::Ptr apply(
      const cslibs_time::Time &until,
      typename sample_set_t::state_iterator_t states) {
    return state_space_ ? model_->apply(data_, state_space_, until, states)
                        : model_->apply(data_, until, states);
  ...
  inline const std::string &getModelName() const { return model_->getName(); }

  inline typename prediction_model_t::Ptr getModel() const { return model_; }
